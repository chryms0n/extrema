...
IMG_NONE_ROOM = pygame.image.load('D:\Bilder\\NoneA.png')
IMG_NONE_HALLWAY = pygame.image.load('D:\Bilder\\NoneB.png')
...
def main():
	...
	for level in range(LEVELTALLY):
		levels.append(Level())
	currentLevel = levels[currentLevelNr]		
	...
	possibleContent = ['start', 'exit', 'heaven']
	while len(possibleContent) != ROOMTALLY:
		possibleContent.append('treasure')
	random.shuffle(possibleContent)	
	for column in rooms:
		for room in column:
			if room and possibleContent[0] == 'start':
				startingRoom = room
			elif room and possibleContent[0] == 'exit':
				room.objs.append(Object('ladder_down', IMG_LADDER_DOWN, True, room))
			elif room and possibleContent[0] == 'heaven':
				room.objs.append(Object('Fountain', IMG_FOUNTAIN, True, room))
			elif room and possibleContent[0] == 'treasure':
				room.objs.append(Object('treasure', IMG_TREASURE, True, room))
			if room and possibleContent[0] != 'start' and possibleContent[0] != 'exit' and possibleContent[0] != 'heaven':
				room.monsters.append(Creature("Goblin", 100, 5, 5, 5, IMG_IMPWARRIOR, room))
			if room:
				del possibleContent[0]		
	

	# creating the hereo
	player = Hero("schufti", 500, 10, 10, 10, IMG_NONE_ROOM, startingRoom)
	...
	while True: 
		for event in pygame.event.get():
			...
			if moved:
				...
				player.checkPosition()	# gets hallway moving options as well as the overall position when leaving a room
...
class Hero(Creature):
	...
	def checkPosition(self):
		...
		if self.room and (self.positionX < 0 or self.positionX > self.room.width - 1 or self.positionY < 0 or self.positionY > self.room.length - 1):
			...
			self.movingOptions = getHallwayMovingOptions(self.currentHallway, (self.positionOverallX, self.positionOverallY))
			if isDoor(NONE , (self.positionOverallX, self.positionOverallY), allDoors):			
...
class Level:
	...
	def __init__(self):
		...
		self.nr = None
		self.roomRowWidth = None
		self.roomRowLength = None
		self.missingRooms = None
		self.roomRect = None
		self.roomtally = None
