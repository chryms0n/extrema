...
IMG_NONE_ROOM = pygame.image.load('D:\Bilder\\Hero15.png')
...
IMG_SKELETON = pygame.image.load('D:\Bilder\Magic_effect1.png')
			...
			for column in level.rooms:
				for room in column:
					if room and level.possibleContent[0] == 'start':
						if currentLevelNr > 0:
							room.attributeObjs['ladder_up'] = Object(LADDER_UP, IMG_LADDER_UP, True, room)
						level.roomKlimb = room
					elif room and level.possibleContent[0] == 'exit' and currentLevelNr < LEVELTALLY - 1:
						room.attributeObjs['ladder_down'] = Object(LADDER_DOWN, IMG_LADDER_DOWN, True, room)
						level.roomDescent = room
					elif room and level.possibleContent[0] == 'heaven':
						room.attributeObjs['fountain'] = Object(FOUNTAIN, IMG_FOUNTAIN, True, room)
					elif room and level.possibleContent[0] == 'treasure':
						room.massObjs.append(Object(TREASURE, IMG_TREASURE, True, room))
					if room and level.possibleContent[0] != 'start' and level.possibleContent[0] != 'exit' and level.possibleContent[0] != 'heaven':
						room.monsters.append(Creature("Goblin", 100, 5, 5, 5, IMG_SKELETON, room))
					if room:
						del level.possibleContent[0]	
			
			# creating the hereo
			player = Hero("schufti", 500, 10, 10, 10, IMG_NONE_ROOM, level.roomKlimb, level)
